...
class Slic {
    private:
		...
		cv::Mat slic_img;
		vector<vector<int>> tmp_neighbor;
    public:
		...
		/*save result image */
		void save_segmente_image(cv::Mat& sourceImage,
			/*const char * resultImage,*/int superpixelNumber, int regularition);
		void save_adjoint_matrix(/*const char * resultImage,*/ int width, int height);

		cv::Mat get_slic_img();
		arma::mat get_slic_matrix();
		vector<vector<int>> get_slic_neighbor();
		int w , h ;
};
