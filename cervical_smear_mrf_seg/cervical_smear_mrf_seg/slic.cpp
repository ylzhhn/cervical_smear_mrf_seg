...
void Slic::init_data(IplImage *image) {
    ...
    for (int i = 0; i < image->width; i++) { 
        ...
        for (int j = 0; j < image->height; j++) {
            cr.push_back(-1);
            dr.push_back(FLT_MAX); // max float
        }
        clusters.push_back(cr); // initial cluster centers 
        distances.push_back(dr);
    }

    /* Initialize the centers and counters. */
	/*
	  Initialize cluster center C_{k} = [l_{k}, a_{k},b_{k},x_{k},y_{k}]
	  by sampling pixels at regular grid steps S.
	  Move cluster centers to the lowest gradient position in a 3*3 neighborhood
	  set label  Lable_{i} = -1 for each pixel i;
	  set distance d_{i} = lim Max 4 each pixel i.
	*/
...
void Slic::create_connectivity(IplImage *image) {
	...
	/*after superpixel divided, there may be some initial labels "-1"*/
	/*need to assign these "-1" labels to new */
	/*new cluster centers will be saved in structure "new_cluster" */
...
void Slic::save_segmente_image(cv::Mat& sourceImage, 
	/*const char * resultImage,*/
	int superpixelNumber, 
	int regularition){
	    IplImage tmp = sourceImage;
		IplImage *image = &tmp;// IplImage 2 IplImage*
		IplImage *lab_image = cvCloneImage(image);
		cvCvtColor(image, lab_image, CV_BGR2Lab);
    
		/* Yield the number of superpixels and weight-factors from the user. */
		w = image->width, h = image->height;
		int nr_superpixels = superpixelNumber ;//100;
		int nc = regularition ; //40;

		//double step = sqrt((w * h) / (double) nr_superpixels);
		int step = (int) sqrt((w * h) / (double) nr_superpixels);
		if (step <= 3)
		{
			step =4 ;
		}
		...
		slic_img = image;//IplImage 2 cv::Mat directly

		Slic::save_adjoint_matrix(/*resultImage,*/ w, h);
...
void Slic::save_adjoint_matrix(/*const char * resultImage,*/ int width, int height)
{

			int nr_superpixels = centers.size()+1; //deal with labels considering label "-1"

			//the number of class£ºcenters.size();
			int ** labelMatrix = new int*[nr_superpixels];
			for(int i = 0; i < nr_superpixels; i++)
				labelMatrix[i] = new int[nr_superpixels];

			for(int i = 0; i < nr_superpixels; i++){
				for(int j = 0; j < nr_superpixels; j++){
					labelMatrix[i][j] = 0;
				}
			}

			int ** tempClusters = new int*[width];
			for(int i = 0; i < width; i++)
				tempClusters[i] = new int[height];

			for(int i = 0; i < width; i++){
				for(int j = 0; j < height-1; j++){
					tempClusters[i][j] = clusters[i][j+1] -clusters[i][j];
				}
				tempClusters[i][height-1] = clusters[i][height-1];
			}
			...
			for(int i = 0; i < width; i++){
				for(int j = 0; j < height; j++){
					if(tempClusters[i][j] !=0){
						centerLabel = clusters[i][j]+1; 
						if(i > 1){	
							leftLabel	= clusters[i-1][j]+1;
							if(centerLabel != leftLabel){
								labelMatrix[centerLabel][leftLabel] = 1;
								labelMatrix[leftLabel][centerLabel] = 1;
							}
						}
						if(i < width-1){	
							rightLabel	= clusters[i+1][j]+1;
							if(centerLabel != rightLabel){
								labelMatrix[centerLabel][rightLabel] = 1;
								labelMatrix[rightLabel][centerLabel] = 1;
							}
						}
						if(j > 1){	
							upLabel	= clusters[i][j-1]+1;
							if(centerLabel != upLabel){
								labelMatrix[centerLabel][upLabel] = 1;
								labelMatrix[upLabel][centerLabel] = 1;
							}
						}
						if(j < height-1){	
							bottomLabel	= clusters[i][j+1]+1;
							if(centerLabel != bottomLabel){
								labelMatrix[centerLabel][bottomLabel] = 1;
								labelMatrix[bottomLabel][centerLabel] = 1;
							}
						}
			...
			//save SLIC superpixel neighbor
			for(int i = 0; i < nr_superpixels; i++){
				vector<int> tmp_row;
				tmp_row.push_back(i-1);
				for(int j = 0; j < nr_superpixels; j++){
					if(labelMatrix[i][j] > 0 ){
						tmp_row.push_back(j-1);
					}
				}
				tmp_neighbor.push_back(tmp_row);
			}

			// free memory
			for(int i = 0; i < width; i++)
				delete tempClusters[i] ;
			for(int i = 0; i < nr_superpixels; i++)
				delete labelMatrix[i] ;
}

cv::Mat Slic::get_slic_img()
{
	return slic_img;
}
arma::mat Slic::get_slic_matrix()
{
	arma::mat slic_m;
	slic_m.zeros(w,h);
	for (int i = 0; i < w; i++){
		for (int j = 0; j < h; j++){
			slic_m(i,j)= clusters[i][j] ;
		}
	}
	return slic_m;
}
vector<vector<int>> Slic::get_slic_neighbor()
{
	return tmp_neighbor;
}